    ...
    return (
      <div style={{ ...J, background: LIGHT, minHeight: '100vh' }}>
        ...
        <div style={{ maxWidth: 600, margin: '0 auto', padding: 'clamp(60px,10vw,100px) 24px', textAlign: 'center' }}>
          ...
          <p style={{ fontSize: '14px', color: MUTED, marginBottom: 24, lineHeight: 1.7 }}>
            Explore the full Voice Control course and book.
          </p>
          <button
      ...
      <div style={{ maxWidth: 1000, margin: '0 auto', padding: 'clamp(48px,6vw,72px) 24px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px,1fr))', gap: 40, marginBottom: 72, alignItems: 'start' }}>
          ...
          <div style={{ background: WHITE, padding: 'clamp(28px,4vw,44px)', border: '1px solid #e8e4dc' }}>
            ...
            {error && <p style={{ fontSize: '13px', color: '#c0392b', marginBottom: 12 }}>{error}</p>}

            <button
              onClick={handleSubmit}
              disabled={loading}
              style={{ width: '100%', padding: '16px', background: loading ? '#888' : DARK, color: WHITE, border: 'none', fontSize: '12px', fontWeight: 700, letterSpacing: '0.18em', textTransform: 'uppercase', cursor: loading ? 'not-allowed' : 'pointer', marginBottom: 10 }}
            >
          ...
          <div>
            <div style={{ marginBottom: 36 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                {[
                  'Why speaking fast sounds nervous',
                  'Why people interrupt certain speakers',
                  'How powerful speakers use pauses',
                  'How sentence endings change perception',
                  'How to sound calmer and more confident instantly',
                ].map((item, i) => (
            ...
            <div style={{ background: WHITE, border: '1px solid #e8e4dc', padding: '28px' }}>
              ...
              {[
                ...
                'Anyone who feels ignored when speaking',
              ].map((item, i) => (
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px,1fr))', gap: 32, marginBottom: 72, alignItems: 'center', borderTop: '1px solid #e8e4dc', paddingTop: 56 }}>
          <div>
            ...
            <p style={{ ...G, fontSize: 'clamp(1.1rem,2.2vw,1.4rem)', fontWeight: 700, color: DARK, margin: '0 0 16px', lineHeight: 1.3 }}>
              Voice Researcher & Speech Coach
            </p>
          ...
          <div style={{ background: DARK, padding: 'clamp(28px,4vw,44px)' }}>
            ...
            <p style={{ fontSize: '14px', color: 'rgba(255,255,255,0.65)', marginBottom: 20, lineHeight: 1.7 }}>
              Before they evaluate your ideas, they react to:
            </p>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10, marginBottom: 24 }}>
              {['pace', 'pauses', 'vocal control', 'tone', 'certainty'].map((item, i) => (
                <span key={i} style={{ background: 'rgba(201,169,110,0.15)', border: '1px solid rgba(201,169,110,0.4)', color: GOLD, fontSize: '12px', fontWeight: 600, padding: '6px 14px', borderRadius: 2 }}>
                  {item}
                </span>
              ))}
            </div>
        ...
        <div style={{ borderTop: '1px solid #e8e4dc', paddingTop: 56, textAlign: 'center' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'center', gap: 16, flexWrap: 'wrap', marginBottom: 36 }}>
            {['calm', 'respected', 'authoritative', 'confident'].map((item, i) => (
              <span key={i} style={{ ...G, fontSize: 'clamp(1rem,2vw,1.2rem)', fontStyle: 'italic', color: GOLD }}>
                {item}{i < 3 ? ' ·' : ''}
              </span>
            ))}
          </div>
