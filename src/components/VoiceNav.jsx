...
function VoiceNav() {
  ...
  const handleMenuClick = (item) => {
    // If it's a path-based item (not section), just navigate
    ...
    // If section item and NOT on home page, navigate to home with hash
    ...
    // If section item and ON home page, scroll to section
    if (item.sectionId && isHomePage) {
      const section = document.getElementById(item.sectionId);
      if (!section) return;
  ...
  const liStyle = `cursor-pointer font-semibold transition-all duration-150 hover:text-[${THEME.accent}]`;
  ...
  const isItemActive = (item) => {
    ...
    // For section items, check if we're on home page and section is active
    ...
    <header style={{ backgroundColor: THEME.bg }}>
      ...
      >
        <a 
          ...
          className="flex items-center gap-1 transition-colors hover:text-[#B68A4C]"
      ...
      <nav
        className={
          isFixed
            ? `fixed left-0 right-0 top-0 z-50 shadow-md`
            : ""
        }
        style={{ 
      ...
      >
        ...
        >
          ...
          <ul className="hidden items-center gap-4 lg:flex xl:gap-6">
            {MENU_ITEMS.map((item, index) => (
              <li key={index}>
                <button
                  type="button"
                  className={`${liStyle} ${isItemActive(item) ? "text-[#B68A4C]" : ""}`}
                  style={{ 
                    ...
                    <li key={index}>
                      <button
                        ...
                        style={{
                          ...
                          backgroundColor: location.pathname === item.path ? THEME.bg : "transparent"
                        }}
                        onMouseEnter={(e) => {
                        ...
                        onMouseLeave={(e) => {
                          e.target.style.backgroundColor = location.pathname === item.path ? THEME.bg : "transparent";
          ...
          <a
            ...
            className="hidden items-center gap-1 font-semibold xl:flex transition-colors hover:text-[#B68A4C]"
        ...
        <div
          ...
          className={`overflow-hidden px-4 transition-[clip-path,opacity,max-height,padding] duration-300 ease-out lg:hidden sm:px-5 ${
            isMenuOpen
              ? "max-h-[600px] border-t pb-4 opacity-100"
        ...
        >
          <ul className="flex flex-col gap-2 pt-3">
            ...
            <li>
              ...
              {isMobileVoiceOpen && (
                <ul 
                  className="ml-3 mt-1 flex flex-col gap-1 border-l-2 pl-3"
          ...
          <a
            href="https://www.instagram.com/sevilvelsha?igsh=MXVoN29zNWIza2NzdQ%3D%3D"
            className="mt-3 inline-flex items-center gap-1 px-3 font-semibold transition-colors hover:text-[#B68A4C]"
            style={{ color: THEME.accent }}
          >
            <FaInstagram className="mt-1" /> @sevil.Velsha
          </a>
          ...
          <a
            href="mailto:sevilvelsha.com"
            className="mt-3 inline-flex items-center gap-1 px-3 font-semibold md:hidden transition-colors hover:text-[#B68A4C]"
            style={{ color: THEME.accent }}
          >
            <MdEmail className="mt-0.5" />
            sevilvelsha.com
          </a>
        </div>
      </nav>
