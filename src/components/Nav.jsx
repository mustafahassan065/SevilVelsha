  ...
  return (
    <header className="bg-bgColor">
      ...
      <nav
        className={
          isFixed
            ? `fixed left-0 right-0 top-0 z-50 bg-bgColor ${hasScrolled ? "shadow-md" : ""}`
            : "bg-bgColor"
        }
      >
          ...
          <ul className="hidden items-center gap-4 lg:flex xl:gap-6">
            {MENU_ITEMS.map((item, index) => (
              <li key={index}>
                <button
                  type="button"
                  className={`${liStyle} ${isItemActive(item) ? "text-green-600" : "text-gray-700"}`}
            ...
            <li ref={dropdownRef} className="relative">
              <button
                type="button"
                className={`${liStyle} flex items-center gap-1 ${isVoiceActive ? "text-green-600" : "text-gray-700"}`}
              ...
              {isDropdownOpen && (
                <ul className="absolute left-0 top-full z-50 mt-2 min-w-[220px] rounded-md border border-gray-100 bg-white py-1 shadow-lg">
                  {VOICE_CONTROL_ITEMS.map((item, index) => (
                    <li key={index}>
                      <button
                        type="button"
                        className={`w-full px-4 py-2.5 text-left text-sm font-semibold transition-colors hover:bg-green-50 hover:text-green-600 ${location.pathname === item.path ? "bg-green-50 text-green-600" : "text-gray-700"}`}
                        onClick={() => {
                          navigate(item.path);
                          setIsDropdownOpen(false);
                        }}
        ...
        <div
          ...
          className={`overflow-hidden bg-bgColor px-4 transition-[clip-path,opacity,max-height,padding] duration-300 ease-out lg:hidden sm:px-5 ${
            isMenuOpen
              ? "max-h-[600px] border-t border-gray-200 pb-4 opacity-100"
        ...
        >
          <ul className="flex flex-col gap-2 pt-3">
              ...
              <li key={index}>
                <button
                  ...
                  className={`w-full rounded-md px-3 py-2 text-left ${liStyle} ${
                    isItemActive(item) ? "bg-green-50 text-green-600" : "text-gray-700"
                  }`}
                  onClick={() => handleMenuClick(item)}
                >
                  {item.label}
            ...
            <li>
              <button
                type="button"
                className={`w-full rounded-md px-3 py-2 text-left ${liStyle} flex items-center justify-between ${isVoiceActive ? "bg-green-50 text-green-600" : "text-gray-700"}`}
              ...
              {isMobileVoiceOpen && (
                <ul className="ml-3 mt-1 flex flex-col gap-1 border-l-2 border-green-100 pl-3">
                  {VOICE_CONTROL_ITEMS.map((item, index) => (
                    <li key={index}>
                      <button
                        type="button"
                        className={`w-full rounded-md px-3 py-2 text-left text-sm font-semibold transition-colors hover:bg-green-50 hover:text-green-600 ${location.pathname === item.path ? "bg-green-50 text-green-600" : "text-gray-700"}`}
                        onClick={() => {
                          navigate(item.path);
                          setIsMenuOpen(false);
